...
# 初始化连接参数
_HOST=( $( __readINI /etc/rsyncd.ini KnowledgeGraph host ) )
_USER=( $( __readINI /etc/rsyncd.ini KnowledgeGraph user ) )
_MODULE=( $( __readINI /etc/rsyncd.ini KnowledgeGraph module ) )
_PASSWORD_FILE=( $( __readINI /etc/rsyncd.ini KnowledgeGraph password_file ) )
